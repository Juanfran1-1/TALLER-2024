...
{INCISO J}
procedure INCISOJ(a:arbol2;min,max:integer;var cant:integer);
var
aux:lista;
begin
	if (a<>nil) then begin
	...
	if (a^.dato.ISBN >= min) and (a^.dato.ISBN <= max) then begin
		...
		INCISOJ(a^.HI,min,max,cant);
		INCISOJ(a^.HD,min,max,cant);
	end	
	else begin
		if (a^.dato.ISBN < min) then 
			INCISOJ(a^.HD,min,max,cant)
...
write('INGRESE VALOR MAX ');readln(max);
cant:=0;
INCISOJ(a2,min,max,cant);
write('LA CANTIDAD DE PRESTAMOS ENTRE EL RANGO DE VALORES ES DE ',cant);
end.
