...
public class ejercicio4 {
    ...
    sistemaxa = new SA("LA PLATA",-34.921,-57.955,2021,3);
    for (x=0;x<3;x++){
        for (y=0;y<12;y++){
            sistemaxa.registrarTemp(GeneradorAleatorio.generarDouble(90), x, y);  
        }
    }
    
    sm=new SM("MAR DEL PLATA",-38.002,-57.556,2020,4);
    for (x=0;x<4;x++){
        for (y=0;y<12;y++){
            sm.registrarTemp(GeneradorAleatorio.generarDouble(90), x, y);
        }
    }
    System.out.println(sistemaxa.toString()); 
